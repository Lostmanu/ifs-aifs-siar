...
def titular(res, sp, dg):
    ...
    if not robustas:
        ...
        L.append(f'**{otra} no sobrevive al análisis de sensibilidad al instrumento; {robustas[0]} sí; medido en {n_st} estaciones SiAR y {n_days} días objetivo de mayo a agosto de 2026, sin invierno.**')
    else:
        L.append(f'**Las dos hipótesis primarias sobreviven al análisis de sensibilidad al instrumento, en {n_st} estaciones SiAR y {n_days} días objetivo de mayo a agosto de 2026, sin invierno.**')
...
def main():
    ...
    L.append('- Bootstrap por días completos (todas las estaciones de cada día remuestreado), bloques circulares de 1, 7 y 14 días, 10.000 réplicas, semilla 20260914. Sensibilidad al instrumento: k ∈ {0,95; 1,00; 1,05} global y 1.000 sorteos de k_i ~ U(0,95; 1,05) por estación, semilla 20260915.')
    ...
    L.append('## 5. Robustez al instrumento\n')
    L.append('Esta es la sección que decide el titular. El modelo es observación medida = k × observación real; el análisis divide la observación por k, de modo que k < 1 representa un sensor que mide de menos.\n')
    ...
    L.append('![Sensibilidad al instrumento](figura_sensibilidad.png)\n')
    ...
    if dg:
        ...
        L.append(f'Calculado el {dg["generado_utc"][:19]} UTC, después de ver los resultados, para interpretar la sensibilidad al instrumento de la sección 5. No entra en las hipótesis, no se corrige por multiplicidad y no cambia ningún veredicto. Los intervalos usan el mismo bootstrap por días completos, semilla y largos de bloque que el análisis principal ({nrep} réplicas; {nmed} para la mediana).\n')
